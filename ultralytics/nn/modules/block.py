...
class RepDEConv(nn.Module):
    def __init__(self, dim):
        super(RepDEConv, self).__init__()
    ...
    def forward(self, x):
        if hasattr(self, 'conv1_1'):
            ...
            res = nn.functional.conv2d(x, self.conv1_5.weight, self.conv1_5.bias, stride=1, padding=1)
    ...
    def switch_to_deploy(self):
        ...
        if hasattr(self, 'bn'):
            bn_weight = self.bn.weight / (self.bn.running_var + self.bn.eps).sqrt()
            weight = weight * bn_weight[:, None, None, None]
            bias = (bias - self.bn.running_mean) * bn_weight + self.bn.bias
            del self.bn

        self.conv1_5.weight = torch.nn.Parameter(weight)
        self.conv1_5.bias = torch.nn.Parameter(bias)
...
class ALDS(nn.Module):
    def __init__(self, inc, ouc) -> None:
        super(ALDS, self).__init__()
        ...
        self.attention = SimAM(1e-4)

    def forward(self, x):
        return self.attention(torch.cat([self.maxpool(x), self.conv(x)], dim=1))
...
class SPP(nn.Module):
    ...
    def __init__(self, c1, c2, k=(5, 9, 13)):
        ...
        c_ = c1 // 2  # hidden channels
        self.cv1 = GSConv(c1, c_, 1, 1)
        self.cv2 = GSConv(c_ * (len(k) + 1), c2, 1, 1)
...
class C2ff(nn.Module):
    ...
    def __init__(self, c1, c2, n=1,e=0.5):
        ...
        self.cv1 = Conv(c1, 2 * self.c, 1, 1)
        self.cv2 = Conv((2 + n) * self.c, c2, 1)  # optional act=FReLU(c2)
        self.m = nn.ModuleList(Bottleneck(self.c, self.c, k=((3, 3), (3, 3)), e=1.0) for _ in range(n))
...
class ADown(nn.Module):
    ...
    def forward(self, x):
        ...
        x2 = self.cv2(x2)
        return self.se(torch.cat((x1, x2), 1))
